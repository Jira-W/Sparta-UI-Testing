...
class SeleniumDemoReg
  ...
  MARITAL_STATUS =    'radio_wrap'
  HOBBY_STATUS =  'reading'
  ...
  def get_selected_country
    @country_element = @chrome_driver.find_element(:id, COUNTRY_DROP_DOWN_LIST)
    @country_element.attribute('value')
  end

  def country_dropdown_list_select(country)
    @select=Selenium::WebDriver::Support::Select.new(@country_element)
    @select.select_by(:text, "#{country}")
  end
  ...
  def dob_month_list_select(month_value)
    @month_element = @chrome_driver.find_element(:id, DOB_MONTH_DROPDOWN_LIST)
    @select_month=Selenium::WebDriver::Support::Select.new(@month_element)
    @select_month.select_by(:text, "#{month_value}")
  end

  def dob_day_list_select(day_value)
    @day_element = @chrome_driver.find_element(:id, DOB_DAY_DROPDOWN_LIST)
    @select_day=Selenium::WebDriver::Support::Select.new(@day_element)
    @select_day.select_by(:text, "#{day_value}")
  end

  def dob_year_list_select(year_value)
  @year_element = @chrome_driver.find_element(:id, DOB_YEAR_DROPDOWN_LIST)
  @select_year=Selenium::WebDriver::Support::Select.new(@year_element)
  @select_year.select_by(:text, "#{year_value}")
  end
  ...
  def check_registration_successful
    @chrome_driver.find_element(:class, REGISTRATION_CONFIRMATION)
  end
end
